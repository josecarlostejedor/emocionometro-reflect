...
<style>
    ...
    /* Fondo Espectacular con Blobs Animados (Igual que la Preview) */
    .stApp {
...
if st.session_state.page == 'votar':
    ...
    for i, emo in enumerate(emociones):
        with cols[i % 4]:
            # Usamos un formato de texto que el CSS detectará para aplicar estilos
            # El icono arriba y el texto abajo
...
# 7. VISTA: RESULTADOS
    ...
    if not df.empty:
        # Gráfico con el color Magenta del logo
        ...
        for _, row in df.iterrows():
            ...
            st.markdown(f"""
                <div class="progress-bar-container">
                    <div style="font-weight:900; font-size:1.2rem; width:180px; text-transform:uppercase; letter-spacing:0.05em; color:#2D2D2D;">{row['emocion']}</div>
...
with st.expander("🛠️ CONFIGURACIÓN"):
    ...
    if pwd == "1234":
        if st.button("⚠️ REINICIAR MARCADOR"):
            reset_db()
            st.success("Datos borrados satisfactoriamente.")
            st.rerun()
