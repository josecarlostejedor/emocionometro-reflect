...
# 2. CSS Maestro: Rediseña los botones REALES de Streamlit para que parezcan tarjetas
st.markdown("""
<style>
    ...
    /* Estilo de los Botones de Emoción (Transformamos el botón real) */
    div.stButton > button {
...
with col_r:
    ...
    st.markdown('<p style="font-size:1.5rem; font-weight:700; margin:0; color:#2D2D2D;">Día de la Educación Física en la Calle</p>', unsafe_allow_html=True)
...
# 5. Navegación (Botones reales de Streamlit)
...
with c_nav1:
    # Botón de resultados con estilo minimalista
    if st.button("📊 RESULTADOS", key="nav_res"): 
        st.session_state.page = 'resultados'
        st.rerun()
with c_nav2:
    # Botón de votar con estilo minimalista
...
if st.session_state.page == 'votar':
    ...
    for i, emo in enumerate(emociones):
        with cols[i % 4]:
            # El botón de Streamlit contiene el icono y el texto
            # Al pulsarlo, se ejecuta la lógica de voto
        ...
        for _, row in df.iterrows():
            ...
            st.markdown(f"""
                <div style="display:flex; align-items:center; gap:1.5rem; margin-bottom:1.2rem; background:rgba(0,0,0,0.03); padding:1.2rem; border-radius:2rem; border: 1px solid rgba(0,0,0,0.05);">
                    <div style="font-weight:900; font-size:1.1rem; width:160px; text-transform:uppercase; letter-spacing:0.05em; color:#2D2D2D;">{row['emocion']}</div>
                    ...
                    <div style="font-weight:900; width:60px; font-size:1.2rem; color:#2D2D2D;">{pct}%</div>
                </div>
            """, unsafe_allow_html=True)
    else:
