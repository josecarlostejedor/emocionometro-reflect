...
# --- 1. CONFIGURACIÓN DE PÁGINA (LIMPIEZA TOTAL) ---
...
# --- Utilidades Query Params (compatibles con varias versiones de Streamlit) ---
def get_qp():
    try:
        return dict(st.query_params)  # Streamlit nuevos
    except Exception:
...
def add_vote(emo):
    # Reintentos simples por si hay bloqueo
    for _ in range(3):
        try:
            conn = sqlite3.connect(DB_PATH, check_same_thread=False, timeout=5)
            c = conn.cursor()
...
# --- 3. LÓGICA DE COMUNICACIÓN (Query Params) ---
qp = get_qp()

# Registrar voto
if "vote" in qp and qp["vote"]:
    valid_ids = {"happy","excited","proud","motivated","loved","tired","bored","sad"}
    if qp["vote"] in valid_ids:
        add_vote(qp["vote"])
    set_qp(view="results")  # ir a resultados tras votar
    st.rerun()

# Reset DB
if qp.get("reset") == "true":
    reset_db()
    set_qp(view="results")
    st.rerun()

current_view = qp.get("view", "vote")
...
# --- 5. RENDERIZADO DE LA INTERFAZ (MISMA UI, BOTONES FUNCIONANDO) ---
def render_ui():
    results = get_results_data()
    total_votos = sum(r['count'] for r in results) if results else 0
    ...
    # NOTA: Usamos HTML real (no escapado) + window.location (no parent)
...
<head>
  ...
  https://fonts.googleapis.com/css2?family=Inter:wght@400;700;900&family=Libre+Baskerville:ital,wght@1,400&display=swap
  <style>
    ...
    .hidden {{ display: none; }}
    img.logo {{ width: 6rem; height: 6rem; object-fit: contain; }}
    @media (min-width: 768px) {{ img.logo {{ width: 8rem; height: 8rem; }} }}
  </style>
</head>
<body>
  <!-- Blobs -->
  ...
  <div class="max-w-6xl mx-auto p-6 md:p-10 min-h-screen flex flex-col">
    ...
    <header class="flex flex-col md:flex-row items-center justify-between gap-8 mb-12 pb-10 border-b border-black/5">
      <div class="flex flex-col md:flex-row items-center gap-6">
        <img src="{logo_b64}" class="logo drop-shadow-sm" alt="logo" />
        <div class="text-center md:text-left">
          ...
          <p class="italic text-pink-600 font-medium font-serif">"Moviendo cuerpos, conectando mentes. La calle es salud mental en movimiento"</p>
        </div>
      </div>
      <div class="flex gap-4">
        ...
        <button onclick="adminReset()" class="p-2 rounded-full border border-black/10 hover:bg-red-500 hover:text-white transition opacity-20 hover:opacity-100" title="Borrar votos"><i data-lucide="rotate-ccw" class="w-4 h-4"></i></button>
      </div>
    </header>

    <!-- Página de Votación -->
    <div id="view-vote" class="{ 'space-y-10' if current_view == 'vote' else 'hidden' }">
      <h2 class="text-3xl md:text-5xl font-bold text-gray-800 tracking-tight">¿Cómo te sientes hoy?</h2>
    ...
    <div id="view-results" class="{ 'space-y-10' if current_view == 'results' else 'hidden' }">
      <div class="flex justify-between items-end">
        <h2 class="text-3xl md:text-5xl font-bold text-gray-800 tracking-tight">Marcador General</h2>
      ...
      <div class="grid md:grid-cols-2 gap-10">
        <div class="glass p-8 rounded-[3rem] min-h-[300px] flex flex-col justify-center">
          {"".join([f'''
          <div class="mb-6">
            <div class="flex justify-between font-bold uppercase text-xs mb-2 opacity-70">
              <span>{e['label']}</span>
        ...
        <div class="flex flex-col justify-center items-center p-10 text-center opacity-20">
          <i data-lucide="bar-chart-3" class="w-24 h-24 mb-6"></i>
          <p class="font-bold uppercase tracking-widest text-xs">Actualización en tiempo real</p>
        </div>
      </div>
    </div>

    <!-- Footer -->
    <footer class="mt-auto pt-20 pb-10 text-center opacity-40 font-bold text-[10px] uppercase tracking-[0.2em]">
  ...
  <script>
    ...
    function changeView(view) {{
      const url url.searchParams.set('view', view);
    ...
    function sendVote(id) {{
      const url = new URL(window.location.href);  // NO parent
      url.searchParams.set('vote', id);
      window.location.href = url.toString();
    }}

    function adminReset() {{
      const ok = confirm("¿Seguro que quieres borrar todos los votos?");
      if (!ok) return;
      const url = new URL(window.location.href);  // NO parent
